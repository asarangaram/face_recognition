...
class FaceRecognitionSchema(LanceModel):
    ...
    vector: Annotated[List[float], Vector(512)]  # Face embeddings, fixed size of 512
...
class FaceVectorStore(LanceTableManager):
    ...
    def add(self, id: str, vector: Annotated[List[float], Vector(512)]):
        self.tbl.add([
             FaceRecognitionSchema(id=id, vector=vector)   
        ])
        pass
    ...
    def searchByVector(self, vector: Annotated[List[float], Vector(512)], threshold:float=0.85, count :int = 1):
        items_found = (
            self.tbl.search(
                vector,
